...
\section{Eines d'analisis}

\subsection{Mecanismes d'especiació}

Per que l'usuari pugui extreure una informacio útil, cal que poguem 
agrupar els individus en espècies. És clar que a la nostra població, 
al igual que a la natura, l'especiació es un fenòmen que cal que emergeixi, 
que no és intrínsec a tot individu. És a dir, que el concepte d'espècie 
no estarà implementat al codi genetic o als mecanismes de funcionament 
comuns dels individus sinó que caldrà observar-ho en el seu comportament.
...
als individus que es reprodueixen sexualment. Còm es pot identificar les 
espècies dels individus que es reprodueixen asexualment? En quin punt es 
considera que dos descendents d'un mateix individus sòn d'una espècie 
diferent?

A més, degut a la seva qualitat emergent, l'especiació no estarà sempre 
ben definida. També pot ser que la selecció a l'hora de reproduir-se 
tingui en compte altres mecanismes que no l'especiació.

Tota aquesta conjuntura ha obligat a deixar de banda el concepte d'espècie 
cap al concepte, una mica més relaxat, de grup reproductiu que, tot i la 
relaxació, segueix proporcionant informació útil. Considerem que un grup 
reproductiu és un conjunt d'individus que es creuen entre sí o provenen 
d'ancestres que s'han creuat entre sí dintre d'un cert perióde de temps 
o d'un cert nombre de generacions.

% TODO: Aclareix-te: o generacions o temps.
...
Primer fonem els dos taxons més antics:
...
I ara discriminem els individus amb les mateixes marques amb una de nova:
...
\begin{tabular}{lcl}
A A A A A A&$\rightarrow$&A A A A A A\\
A A A A A B&$\rightarrow$&A A A A A B\\
A A A A B B&$\rightarrow$&A A A A B B\\
...
\item	Generar un nou taxó (Per individuus generats espontàneament)
\item	Envellir les marques i discriminar la població que comparteix el mateix taxó
\item	Determinar el grau de parentesc entre dos individuus
\end{itemize}
