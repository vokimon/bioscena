...
\item   {\tt bool CTopologia::esPosicioValida(t\_posicio id)}:
    Una funció per saber si un identificador de posició és vàlid. 
	Per defecte, els identificadors vàlids van de $0$ a $N-1$ on N és
...
perquè l'agent els faci servir per configurar-se.
...
anomenarem aquí, per entendre'ns, {\tt CNouControl}.
\item Crear un constructor per fer inicialitzacions d'atributs 
independents de la informació genètica i on s'agregui a 
l'identificador de tipus {\tt m\_tipus} la porció de nom del 
