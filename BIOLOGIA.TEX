...
quelcom independent sinó que estaven en una estructura superior 
formant els {\bf cromosomes}. 

Això implica, els gens que controlen cada caràcter no s'hereden de
...
({\bf locus}) i, quan reconvinem el material genètic, els gens més 
propers tenen molta més probabilitat d'heredar-se conjuntament.

Anomenem {\bf cariotip} al conjunt de cromosomes que té una espècie 
(en quant a nombre i morfologia).

Els cromosomes que diem que són homòlegs tènen la mateixa morfologia
...
Les {\bf mutacions cromosòmiques estructurals} són les no intervenen cromosomes
integres sino fragments d'aquests.
...
\item	La {\bf inversió} consisteix en el canvi de sentit d'un
...
La introducció d'organismes diplonts o poliplonts suposaria implementar un mecanisme
de resolució del fenotip heterozigòtic als gens homòlegs. També sorgirien alguns 
altres problemes, relacionats amb el creuament, que es comenten en el següent apartat.
...
%% TODO: Contrastar l'afirmació anterior amb un expert
...
\item	En duplicar-se la cadena, per una divisió cel·lular, si encara no s'ha 'corregit´ 
	...
	el genoma primitiu.
\item	Tant si es repara com si es queda la mutació sense reparar, 50\% la probabilitat de traspasar-la a un descendent.
